 ...
 * Ink band, edge to edge. Prices refresh once a day (end of day), not live -
 * from /api/ticker (Twelve Data ETF proxies + CoinGecko crypto, cached ~24h).
 * The curated SEED is the first paint + fallback so it never renders broken.
 * ▲ green = up, ▼ red = down.
 */
...
export function TickerBar() {
  ...
  // fetch once; the API is cached ~24h so this is the daily snapshot
  useEffect(() => {
    let alive = true;
    fetch("/api/ticker")
